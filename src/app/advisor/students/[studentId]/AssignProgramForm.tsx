...
export default function AssignProgramForm({
  ...
  const t = useTranslations('assignProgram')
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError(null)
    if (!selectedProgramId) { setError(t('selectRequired')); return }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-6 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
      ...
      <div>
        <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">{t('programLabel')}</label>
        <select
      ...
      {definitions.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400">{t('dueDatesInstruction')}</p>
          {definitions.map((def) => (
