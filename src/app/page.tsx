  ...
  if (!profileRow) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        <p className="text-zinc-500 dark:text-zinc-400">{t('profileNotSetup')}</p>
        <Link
          href="/profile"
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-6 p-6 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
        <div className="flex-1 space-y-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1 mt-3 text-sm">
            <div>
              <span className="text-zinc-400 dark:text-zinc-500">{t('advisor')}</span>
              <p className="font-medium">{profileRow.advisor_id ?? '—'}</p>
            </div>
            <div>
              <span className="text-zinc-400 dark:text-zinc-500">{t('department')}</span>
              <p className="font-medium">{profileRow.department ?? '—'}</p>
            </div>
            <div>
              <span className="text-zinc-400 dark:text-zinc-500">{t('stage')}</span>
              <p className="font-medium">{tStages(stage as any)}</p>
            </div>
            {profileRow.start_date && (
              <div>
                <span className="text-zinc-400 dark:text-zinc-500">{t('started')}</span>
                <p className="font-medium">
                  {new Date(profileRow.start_date).toLocaleDateString(locale, {
                    year: 'numeric',
                    month: 'short',
                  })}
                </p>
            ...
            {profileRow.expected_graduation && (
              <div>
                <span className="text-zinc-400 dark:text-zinc-500">{t('expectedGraduation')}</span>
                <p className="font-medium">
                  {new Date(profileRow.expected_graduation).toLocaleDateString(locale, {
                    year: 'numeric',
                    month: 'short',
                  })}
                </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard label={t('totalMilestones')} value={total} />
        <StatCard label={t('completed')} value={completed} sub={t('percentDone', { percent: percentage })} />
        <StatCard label={t('overdue')} value={overdue} sub={overdue === 0 ? t('overdueNone') : t('overdueAttention')} />
        <StatCard label={t('monthsToGraduation')} value={monthsLeft} sub={t('approx')} />
      </div>
      ...
      <div className="rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6">
        ...
        {upcoming.length === 0 ? (
          <p className="text-sm text-zinc-400 dark:text-zinc-500">{t('noUpcoming')}</p>
        ) : (
            ...
            {upcoming.map((m) => (
              <li key={m.id} className="flex items-center justify-between gap-4 text-sm">
                ...
                <span className="text-zinc-400 dark:text-zinc-500 whitespace-nowrap">
                  {new Date(m.dueDate).toLocaleDateString(locale, { month: 'short', day: 'numeric' })}
                </span>
              </li>
            ))}
          </ul>
